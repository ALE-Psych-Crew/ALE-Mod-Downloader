...
var modDownloaderHint:FlxText = null;
...
function openDownloader()
{
    ...
    canSelect = false;
    FlxG.sound.play(Paths.sound('confirmMenu', true));
    FlxTween.tween(modDownloaderHint, {alpha: 0}, 0.15, {ease: FlxEase.cubeOut});
    FlxTimer.wait(0.25, () -> {
        CoolUtil.switchState(new CustomState('ModDownloaderState'));
    });
}
...
modDownloaderHint.y = FlxG.height * 0.76;
add(modDownloaderHint);

if (ClientPrefs.data.flashing)
    FlxTween.tween(modDownloaderHint, {alpha: 0.45}, 1.2, {ease: FlxEase.sineInOut, type: FlxTweenType.PINGPONG});
